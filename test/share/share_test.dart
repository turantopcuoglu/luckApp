...
void main() {
  ...
  testWidgets('StoryCard skor, tarih, 5 kategori ve markayı gösterir', (
    ...
    await tester.pumpWidget(
      Directionality(
        ...
        child: StoryCard(sonuc: sonuc, dil: AppDil.tr),
      ),
    );

    expect(find.text('${sonuc.genelSkor}'), findsWidgets);
    expect(
      find.text(TrStrings.tarihMetni(AppDil.tr, sonuc.gun)),
      findsOneWidget,
    );
    ...
    expect(find.text(ShareStrings.genelSkor(AppDil.tr)), findsOneWidget);
  });
  ...
  testWidgets('kilitli kategorilerin skoru story kartına sızmaz', (
    ...
    expect(find.byIcon(Icons.lock_rounded), findsNWidgets(kilitliler.length));
    ...
    for (final LuckCategory k in kilitliler) {
      ...
      if (!gorunenSkorlar.contains(gizliSkor)) {
        expect(
          find.text('$gizliSkor'),
          findsNothing,
          reason: '${k.etiket(AppDil.tr)} skoru paylaşım kartına sızdı',
        );
      }
    }
  });
  ...
  testWidgets('kartPngUret 1080x1920 boyutunda geçerli PNG üretir', (
    ...
    await tester.runAsync(() async {
      ...
      final List<int> png = await servis.kartPngUret(
        StoryCard(sonuc: sonuc, dil: AppDil.tr),
      );
